...
namespace Delegates.Reports
{
    public class HtmlReportMaker
    {
            ...
            string caption)
        {
            ...
            result.Append("<ul>");
            result.Append($"<li><b>{"Temperature"}</b>: {makeStatistics(data.Select(z => z.Temperature)).ToString()}");
            result.Append($"<li><b>{"Humidity"}</b>: {makeStatistics(data.Select(z => z.Humidity)).ToString()}");
            result.Append("</ul>");
    ...
    public class MarkdownReportMaker
    {
            ...
            string caption)
        {
            ...
            result.Append("");
            result.Append($" * **{"Temperature"}**: {makeStatistics(data.Select(z => z.Temperature)).ToString()}\n\n");
            result.Append($" * **{"Humidity"}**: {makeStatistics(data.Select(z => z.Humidity)).ToString()}\n\n");
            result.Append("");
            return result.ToString();
        }
